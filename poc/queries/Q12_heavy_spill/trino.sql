...
-- Dialect: Trino 481
-- approx_percentile(x, p) replaces Doris PERCENTILE_APPROX(x, p).
-- Trino spills to /data/trino/spill when per-node memory limit is hit.
...
SELECT
    ...
    approx_percentile(total_revenue, 0.50) AS p50_revenue,
    approx_percentile(total_revenue, 0.95) AS p95_revenue,
