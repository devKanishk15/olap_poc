...
-- Tests: memory management under pressure; multi-level GROUP BY + window over large dataset
-- Expected to spill on all engines at 8 GB — flag spill depth and latency
...
SELECT
    ...
    PERCENTILE_APPROX(total_revenue, 0.5)  AS p50_revenue,
    PERCENTILE_APPROX(total_revenue, 0.95) AS p95_revenue,
