-- GQ02 — Filtered aggregate: only active images (pc_item_img_status = 'A')
...
SELECT
    ...
    min(pc_item_image_update_date)        AS earliest_active,
    max(pc_item_image_update_date)        AS latest_active
FROM s3(
